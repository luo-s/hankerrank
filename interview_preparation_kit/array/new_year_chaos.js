/*
It is New Year's Day and people are in line for the Wonderland rollercoaster 
ride. Each person wears a sticker indicating their initial position in the 
queue from 1 to n. Any person can bribe the person directly in front of them 
to swap positions, but they still wear their original sticker. 

One person can bribe at most two others.

Determine the minimum number of bribes that took place to get to a given queue 
order. Print the number of bribes, or, if anyone has bribed more than two 
people, print 'Too chaotic'.
*/

// https://www.hackerrank.com/challenges/new-year-chaos/problem?isFullScreen=true&h_l=interview&playlist_slugs%5B%5D=interview-preparation-kit&playlist_slugs%5B%5D=arrays

function minimumBribes(q) {
  // count bribes from the end of the array
  let bribes = 0;
  for (let i = q.length - 1; i >= 0; i--) {
    // check if the person has bribed more than twice
    if (q[i] > i + 3) {
      console.log("Too chaotic");
      return;
    } else if (q[i] === i + 2) {
      // check if the person bribed once
      bribes++;
      // swap back
      [q[i], q[i + 1]] = [q[i + 1], q[i]];
      // adjust the iterator
      i++;
    } else if (q[i] === i + 3) {
      // check if the person bribed twice
      bribes += 2;
      // swap back
      [q[i], q[i + 1], q[i + 2]] = [q[i + 1], q[i + 2], q[i]];
      // adjust the iterator
      i += 2;
    }
  }
  console.log(bribes);
  return;
}
